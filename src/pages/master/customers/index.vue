...
const onSubmitModal = async () => {
    if (modeModal.value === "add") {
        ...
        await _apiCustomer.create(body, async (response) => {
            if (response.statusCode === 200) {
                modalAlert.value = {
                    status: true,
                    title: "สำเร็จ",
        ...
        await _apiCustomer.update(body, rowAction.value.id, async (response) => {
            if (response.statusCode === 200) {
                modalAlert.value = {
                    status: true,
                    title: "สำเร็จ",
                ...
                <div class="py-4 flex flex-wrap">
                    ...
                    <div class="basis-1/2 px-3 space-y-2 mb-3">
                        ...
                        <select v-model="formModal.in_province"
                            ...
                            <option v-for="(item, index) in ddl.province" :key="index" :value="item.id">
                                {{ item.name_th }}
                            </option>
                        </select>
                    </div>
                    <div class="basis-1/2 px-3 space-y-2 mb-3">
                        ...
                        <select v-model="formModal.in_district"
                            ...
                            <option value="">------เลือก------</option>
                            <option v-for="(item, index) in ddl.district" :key="index" :value="item.id">
                                {{ item.name_th }}
                            </option>
                        </select>
                    </div>
